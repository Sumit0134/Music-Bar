...
async function main() {
    // Fetch songs for the "songs" folder to initialize
    ...
    if (firstAlbum) {
        ...
        // Fetch songs for the first album
        songs = await getSongs(`songs/${folder}`);
        
        // Play the first song in the list if available
        if (songs.length > 0) {
    ...
    document.querySelector(".volume > img").addEventListener("click", (e) => {
        if (e.target.src.includes("svgs/volume.svg")) {
            ...
            document.querySelector(".range").getElementsByTagName("input")[0].value = 100; // Reset to default volume
        }
    });
}
